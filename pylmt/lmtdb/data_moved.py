...
# -t <ost>   The name of the OST to examine
...
def process_args(main=False):
    ...
    parser.add_argument('-t', '--ost', default=None, type=str, help='Name of the OST to examine')
...
def validate_args(args):
    if (((args.begin == None) and (args.end != None)) or
        ...
        print "data_moved.validate_args(): Please provide both a begin and an end argument (or neither for the default)"
        return(None)
...
def do_action(args, bulk):
    ...
    if args.ost != None:
        ...
        for oss in bulk.OSSs:
            if args.ost in oss.OSTDict:
                ost = oss.OSTs[oss.OSTDict[args.ost]]
                break
        if ost == None:
            print "data_moved.do_action(): Warning - OST %s not found" % args.ost
            return
        if args.both == True:
            print "%.0f %s" % (np.sum(ost.Both.Values)/scale, scale_str)
        if args.read == True:
            print "%.0f %s" % (np.sum(ost.Read.Values)/scale, scale_str)
        if args.write == True:
            print "%.0f %s" % (np.sum(ost.Write.Values)/scale, scale_str)
        return
    if args.oss != None:
        scale = 1024*1204
        scale_str = "MiB"
        ...
        if args.both == True:
            print "%.0f %s" % (np.sum(oss.Both.Values)/scale, scale_str)
        if args.read == True:
            print "%.0f %s" % (np.sum(oss.Read.Values)/scale, scale_str)
        if args.write == True:
            print "%.0f %s" % (np.sum(oss.Write.Values)/scale, scale_str)
        return
    scale = 1024*1024*1024
    scale_str = "GiB"
    stepDiffs = bulk.Steps.Diff
    if args.both == True:
        values = bulk.Both.Values[1:]
        print "%.0f %s" % (np.sum(values*stepDiffs)/scale, scale_str)
    if args.read == True:
        values = bulk.Read.Values[1:]
        print "%.0f %s" % (np.sum(values*stepDiffs)/scale, scale_str)
    if args.write == True:
        values = bulk.Write.Values[1:]
        print "%.0f %s" % (np.sum(values*stepDiffs)/scale, scale_str)
