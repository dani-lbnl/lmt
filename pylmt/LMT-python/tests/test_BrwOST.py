...
from pyLMT import LMTConfig, Timestamp, Graph, BrwOST
...
def do_main(args):
    ...
    if args.showStats == True:
        ...
        return
    BO.getData(beginTimestamp, endTimestamp, fsrc['conn'], args.stat)
    return(BO)
...
def do_action(args, BO):
    ...
    if not args.bin is None:
        ...
        plt.title("%s %s aggregate I/O" % (dayStr, Bins.name))
    ...
    for Bins in BO.Bins:
        Plots = []
        if args.read == True:
            Plots.append({'values': (BO.Read[Bins.id].Values[:,-1] -
                                     BO.Read[Bins.id].Values[:,0]),
                          'label':'read',
                          'color':'r'})
            if args.write == True:
                Plots.append({'values': (BO.Write[Bins.id].Values[:,-1] -
